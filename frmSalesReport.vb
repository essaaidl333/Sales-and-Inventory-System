...
Public Class frmSalesReport
    ...
    Sub fillUsers()
        Try
            ...
            adp.SelectCommand = New SqlCommand("SELECT distinct (name) FROM Registration", con)
            ds = New DataSet("ds")
            adp.Fill(ds)
            dtable = ds.Tables(0)
    ...
    Private Sub btnGetData_Click(sender As System.Object, e As System.EventArgs) Handles btnGetData.Click
        Try
            ...
            ctn = "select InvoiceInfo.InvoiceNo,Registration.Name from InvoiceInfo,Registration where InvoiceInfo.UserID=Registration.UserID and Registration.Name= '" + ComboBox1.Text + "'  and InvoiceDate between @d1 and @d2"


                con = New SqlConnection(cs)
                con.Open()
                ' Dim ctn As String = "select InvoiceInfo.InvoiceNo from InvoiceInfo where InvoiceDate between @d1 and @d2"
            ...
            MyCommand.CommandText = "SELECT * FROM InvoiceInfo , Customer,Registration where  InvoiceInfo.CustomerID = Customer.ID and InvoiceInfo.UserID=Registration.UserID and Registration.Name= '" + ComboBox1.Text + "' and InvoiceDate between @d1 and @d2 order by InvoiceDate"
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT CONVERT(varchar(10),YEAR(InvoiceDate)) AS Year, SUM(GrandTotal) AS GrandTotal,Registration.Name FROM Registration, InvoiceInfo where InvoiceInfo.UserID=Registration.UserID and Registration.Name= '" + ComboBox1.Text + "' and InvoiceDate between @d3 and @d4 GROUP BY YEAR(InvoiceDate),Registration.Name ORDER BY Year ", con)
