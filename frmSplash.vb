...
Public Class frmSplash
    ...
    Private Sub frmSplash1_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            '    con.Open()
            '    Dim ct As String = "select RTRIM(ActivationID), ActivationTime from Activation where HardwareID=@d1 and SerialNo=@d2"
            ...
            '    cmd.Parameters.AddWithValue("@d3", Encrypt(DateTimePicker2.Value))
            '    rdr = cmd.ExecuteReader()
            '    If rdr.Read() Then
            '        txtActivationID.Text = Decrypt(rdr.GetValue(0))
            '        TextBox3.Text = Format(rdr.GetValue(1), "dd-MM-yyyy")
            '        DateTimePicker2.Value = rdr.GetValue(1)
            '    End If
